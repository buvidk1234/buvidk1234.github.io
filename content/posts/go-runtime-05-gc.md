...
> 本文源码基线为 **Go 1.26.2、Linux/amd64，默认 GOEXPERIMENT**。Go 1.26.2 源码包含 build-tag 隔离的 Green Tea GC 实现，但默认 `GOEXPERIMENT` 下 `goexperiment.GreenTeaGC == false`；本文主线分析默认标记器，不把实验实现混入结论。

Go GC 的核心不是“三色”这个名词，而是在用户 goroutine 持续修改对象图时，怎样并发找出仍然可达的对象，并把暂停时间与 CPU、内存开销控制在目标范围内。
...
标记工作不会全部堆在一个全局队列中。每个 P 可以持有本地 `gcWork` 缓冲，减少全局竞争；本地积累到一定程度再与全局工作池交换。
...
默认标记器的 [`gcDrain`](https://github.com/golang/go/blob/go1.26.2/src/runtime/mgcmark.go#L1239) 根据 flags 决定是否可阻塞、是否只处理根、是否按时间或字节配额退出。大体循环为：先消费 root jobs，再从本地/全局 work buffer 取灰对象，通过 `scanobject` 扫描；退出前刷新本地工作计数。

源码中的 Green Tea 文件使用不同的 span/对象批处理策略。因为它由 `goexperiment.greenteagc` build tag 选择，阅读时必须先执行 `go env GOEXPERIMENT`，不能把两套 `gcDrain` 辅助结构拼成一个不存在的默认算法。
...
当其他 worker 缺工作而本地 buffer 较满时，[`handoff`](https://github.com/golang/go/blob/go1.26.2/src/runtime/mgcwork.go#L509) 把一半对象移到新 buffer，并把原 buffer 放入全局 full 栈。这是一种批量 work sharing，而不是每个对象都进入单一全局队列。
...
在 Go 1.26.2 的各 `mallocgc*` 分支中，判断 `writeBarrier.enabled` 后调用 [`gcmarknewobject`](https://github.com/golang/go/blob/go1.26.2/src/runtime/mgcmark.go#L1748)。这比笼统说“mallocgc 最后统一涂黑”更准确：不同对象布局和专用分配函数在各自完成 heap bits 初始化后发布标记。
...
goroutine 分配内存
  -> 累积 assist debt
  -> 债务超过阈值
  -> 暂停继续分配，执行一部分 gcDrain
  -> 完成对应标记工作后恢复用户代码
```

这形成负反馈：制造更多 GC 压力的 goroutine 承担更多标记工作。不过 assist 也会增加请求延迟，调优时不能只看总 GC CPU。
...
`runtime.GC()` 请求并等待一轮 GC 达到相应完成条件，但不承诺把进程 RSS 立即降到最低。对象死亡、span 变为空闲、页被 scavenger 处理、操作系统更新 RSS 是不同阶段。

因此手动频繁调用 GC 通常不是常规优化。应先确认问题是对象可达、堆复用、scavenge 延迟，还是容器统计口径。
...
| 默认标记循环 | [`runtime/mgcmark.go`](https://github.com/golang/go/blob/go1.26.2/src/runtime/mgcmark.go#L1239) |
| Green Tea实验实现 | [`runtime/mgcmark_greenteagc.go`](https://github.com/golang/go/blob/go1.26.2/src/runtime/mgcmark_greenteagc.go) |
...
`gctrace` 适合观察 cycle、堆目标和 CPU 概况；`gcpacertrace` 是内部调试输出，字段可能跨版本变化；trace 用于把 STW、mark assist 和 goroutine 延迟放到同一时间轴。实验必须记录 `go version`、`GOEXPERIMENT`、`GOGC`、`GOMEMLIMIT` 和容器限制，否则数据不可复现。
