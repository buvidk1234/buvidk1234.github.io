...
第三，goroutine 的入口不是简单地“调用 f”。runtime 会构造一段调度上下文，让它第一次被 `gogo` 恢复时跳到 `f`，并且在 `f` 返回后进入 `goexit` 做清理。
...
系统调用返回后，runtime 需要让当前 `G` 重新获得执行 Go 代码的资格。这里有两条常见路径。

第一条是快速路径：原来的 `P` 还在，或者能立刻拿到空闲 `P`。这时当前 `G` 可以从 `_Gsyscall` 回到 `_Grunning`，继续在当前线程上执行。
...
第二条是慢速路径：没有可用 `P`。这时当前 `G` 不能继续执行 Go 代码，只能变回 `_Grunnable`，放入全局运行队列，当前 `M` 也可能休眠或去找别的工作。
